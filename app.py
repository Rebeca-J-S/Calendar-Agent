...
# Dibujar el historial en la interfaz
...
if prompt := st.chat_input("¿Qué agendamos hoy?"):
    ...
    # 1. Recorrer los mensajes anteriores para armar un "resumen" de la charla
    ...
    # 2. Unir el historial con el mensaje actual para darle todo el contexto al agente
    ...
    # Guardar el mensaje en Streamlit
    st.session_state.messages.append({"role": "user", "content": prompt})

    with st.chat_message("assistant"):
        with st.spinner("Pensando y revisando el calendario..."):
            try:
                # Enviar al agente el prompt "dopado" con toda la memoria
